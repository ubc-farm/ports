import {extend, copyAndExtendArray} from '../util.js'
...
export default class Configurator {
	constructor(
		parentModule, defaultContainer, configureOptions, pixelRatio = 1
	) {
	...
	_handleArray(arr, value, path) {
		if (typeof arr[0] === 'string' && arr[0] === 'color') {
			...
			if (arr[1] !== value) { this.changedOptions.push({ path, value }); }
		}
		else if (typeof arr[0] === 'string') {
			this._makeDropdown(arr, value, path);
			if (arr[0] !== value) { this.changedOptions.push({ path, value }); }
		}
		else if (typeof arr[0] === 'number') {
			this._makeRange(arr, value, path);
			if (arr[0] !== value) { this.changedOptions.push({ path, value: Number(value) }); }
		}
	}
